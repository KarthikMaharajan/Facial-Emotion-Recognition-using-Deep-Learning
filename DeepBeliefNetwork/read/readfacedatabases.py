...
def readKaggleCompetitionSmallDataset(equalize, train):
  # Got data from here:
  ...
  labels = []
  i = 0
  with open('/Users/KarthikMaharajan/Documents/PRproject/DataSets/dbndataset/train.csv', 'rb') as f:
    reader = csv.reader(f)
    for row in reader:
      if i > 0:
        # print row
...
def readKaggleCompetitionUnlabelled():
  data = []
  i = 0
  with open('/Users/KarthikMaharajan/Documents/PRproject/DataSets/dbndataset/test.csv', 'rb') as f:
    reader = csv.reader(f)
    for row in reader:
      if i > 0:
        # print row
...
def makeKagglePlot():
  data, labels = readKaggleCompetitionSmallDataset(False)
  data, labels = shuffle(data, labels)
  labels = np.argmax(labels, axis=1)
  print np.unique(labels)
